...
public abstract class MultiJsonDataLoader
	...
	protected Map<Identifier, List<JsonElement>> prepare(ResourceManager resourceManager, Profiler profiler) {
		...
		Iterator<Identifier> resources = resourceManager.findResources(this.dataType, (path) -> {
			return path.endsWith(".json");
		}).iterator();
		Set<String> resourcesHandled = new HashSet<>();
		while (resources.hasNext()) {
			Identifier identifier = (Identifier) resources.next();
			String string = identifier.getPath();
			Identifier identifier2 = new Identifier(identifier.getNamespace(),
					string.substring(i, string.length() - FILE_SUFFIX_LENGTH));
						...
						try {
							...
							try {
								...
								try {
									...
									try {
										JsonElement jsonElement = JsonHelper.deserialize(this.gson, (Reader) reader,
												JsonElement.class);
										if (jsonElement != null) {
											...
											LOGGER.error("Couldn't load data file {} from {} as it's null or empty", identifier2,
													identifier);
										}
